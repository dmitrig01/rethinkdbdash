...
Term.prototype.tableCreate = function(table, options) {
    ...
    if (helper.isPlainObject(options)) {
        for(var key in options) {
            if ((key !== "primaryKey") && (key !== "durability") && (key !== "datacenter")) {
...
Term.prototype.indexCreate = function(name, fn, options) {
    ...
    return new IndexCreate(this._r, this, name, fn, options);
}
...
Term.prototype.object = function() {
    ...
    var args = helper.toArray(arguments);
    return new ReqlObject(this._r, args);
}
...
Term.prototype.toString = function() {
    return Error.generateBacktrace(this._self, 0, null, [], {indent: 0, extra: 0}).str;
}
        ...
        for(var i=0; i<value.length; i++) {
            if (isJSON === true) {
                ...
                if (!temp.isJSON) {
                    ...
                    result.push(new Term(_r).json(JSON.stringify(temp.value)));
                }
            }
        }
        if (isJSON === true) {
                ...
                if (isJSON) {
                    ...
                    if (temp.isJSON === false) {
                        for(var k=0; k<previousKeys.length; k++) {
                            result[previousKeys[k]] = new Term(_r).json(JSON.stringify(value[previousKeys[k]]));
                        }
